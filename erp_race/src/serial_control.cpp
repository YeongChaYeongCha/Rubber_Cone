...
void serialCallback(const cmd_erp::erp_control_values::ConstPtr& msg) {
	int steer_total = 0;
	unsigned int speed_total = 0;
	speed_total = abs(msg->velocity)*10;
	printf("msg->velocity : %d\n", msg->velocity);
	if(msg->velocity < 255 && msg->velocity > 0) { // forward
		gear = 0x00;
		speed_1 = speed_total;
		speed_0 = 0x00;
		front_brake = 0x00;
		estop = 0x00;
	} else if(msg->velocity > -255 && msg->velocity < 0) { // backward
		gear = 0x02;
		speed_1 = speed_total;
		speed_0 = 0x00;
		front_brake = 0x00;
		estop = 0x00;
	} else if(msg->velocity == 0) { // stop
		speed_0 = 0x00;
		speed_1 = 0x00;
		gear = 0x01;
		front_brake = 0x33;   // 99 
